...
import { createSchoolNews } from '../../controllers/school_news.controller';
import CustomButton from '../../general_widgets/button';

const CreateSchoolNewsModal = ({ show, onHide }) => {
  ...
  const handleSubmit = (values) => {
    ...
    const data = {title:values.title,description:htmlContent,file}
     createSchoolNews(data).then((response)=>{
      if(response === true){
        onHide()
        setLoading(false)
      }
  ...
  return (
    <Modal show={show} className='' size='xl' onHide={onHide}>
      <Container>
        <Modal.Header>
          <Heading text={'Create school news'} />
            ...
            {({ handleSubmit, handleChange, values, touched, errors }) => (
              <Form noValidate onSubmit={handleSubmit}>
                ...
                <Stack direction='horizontal'>
                  <CustomButton className={"mt-3"} loading={loading} text={"Create news"}/>
                </Stack>
              </Form>
            )}
          </Formik>
...
export default CreateSchoolNewsModal;
