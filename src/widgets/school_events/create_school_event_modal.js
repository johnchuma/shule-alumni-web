...
import { createSchoolEvent } from '../../controllers/school_event_controller';
import CustomButton from '../../general_widgets/button';

const CreateSchoolEventModal = ({ show, onHide }) => {
  ...
  const handleSubmit = (values) => {
    ...
    const data = {title:values.title,file,description:htmlContent}

  createSchoolEvent(data).then((response)=>{
    if(response === true){
      onHide()
      setLoading(false)
    }
  ...
  return (
    <Modal show={show} className='' size='xl' onHide={onHide}>
      <Container>
        <Modal.Header>
          <Heading text={'Create school event'} />
          ...
          <Formik initialValues={{ title: '' }} onSubmit={handleSubmit} validationSchema={schema}>
            {({ handleSubmit, handleChange, values, touched, errors }) => (
              <Form noValidate onSubmit={handleSubmit}>
                ...
                <CustomButton className={"mt-3"} loading={loading} text={"Create event"}/>
...
export default CreateSchoolEventModal;
