...
import { updateSchoolEvent } from '../../../controllers/school_event_controller';




const UpdateSchoolEventModal = ({ show, onHide, event }) => {
  ...
  useEffect(() => {
    if (event !== null) {
      ...
      const contentState = convertFromHTML(event.description);
  ...
  const handleSubmit = (values) => {
    ...
    const data = {title:values.title,image:event.image,description:htmlContent,file}; 
    console.log("hello there")
    updateSchoolEvent(data,event.uuid).then((response)=>{
            onHide();
    });
  ...
  return (
    <Modal show={show} className='' size='xl' onHide={onHide}>
      <Container>
        <Modal.Header>
          <SubHeading text={'Update School event'} />
        ...
        <Modal.Body>
          <Formik initialValues={{ title: event ===null?"": event.title }} onSubmit={handleSubmit} validationSchema={schema}>
...
export default UpdateSchoolEventModal;
